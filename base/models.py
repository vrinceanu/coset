...
class SectionPage(Page):
    ...
    page_description = (
        "A section header page that organizes content without having its own body text."
    )
...
class HomePage(Page):
    ...
    hero_title = models.CharField(
        max_length=255, blank=True, default="Welcome to the COSET website!"
    )
    hero_cta_text = models.CharField(
        max_length=255, blank=True, default="Explore our courses and people"
    )
    ...
    content_panels = Page.content_panels + [
        ...
        InlinePanel(
            "testimonial_posts", label="Testimonial Posts", max_num=3, min_num=0
        ),
    ]
...
class UndergraduateProgramIndexPage(Page):
    ...
    def get_context(self, request):
        ...
        context = super().get_context(request)
        programs = Program.objects.filter(active=True, degree_conferred="BS").order_by(
            "department"
        )
...
class GraduateProgramIndexPage(Page):
    ...
    def get_context(self, request):
        ...
        programs = (
            ...
            .filter(
                Q(degree_conferred="MS")
                | Q(degree_conferred="4+1")
                | Q(degree_conferred="PHD")
            )
            .order_by("department", "degree_conferred")
        )
        context["programs"] = programs
        return context
...
class PersonIndexPage(RoutablePageMixin, Page):
    ...
    def person_view(self, request, slug):
        ...
        return self.render(
            request, context_overrides={"person": person}, template="person_page.html"
        )
...
class PostPage(Page):
    ...
    author = models.CharField(
        max_length=200,
        blank=True,
        help_text="For News: the person(s) involved in the news item. For Events: the organizer or featured speaker.",
    )
    location = models.CharField(
        max_length=300,
        blank=True,
        help_text="For Events: the event location. For News: optional location associated with the news item.",
    )
    url = models.URLField(
        blank=True,
        help_text="Link to RSVP / registration form/ Optional Google Maps or venue URL.",
    )
    unit = models.CharField(
        max_length=100,
        blank=True,
        help_text="Optional unit/department associated with this news item.",
    )
    # --- core fields
    summary = models.TextField(
        max_length=400,
        help_text="Short summary shown on the index card (max 400 chars).",
    )
    body = RichTextField()
...
def get_dept_sidebar_context(page):
    ...
    if isinstance(page, DepartmentPage):
        ...
        dept_page = DepartmentPage.objects.ancestor_of(page).order_by("-depth").first()
    if not dept_page:
        return {}
...
class DepartmentFacultyPage(Page):
    ...
    def get_context(self, request):
        ...
        context["people"] = Person.objects.filter(
            active=True, department=dept_abbrev
        ).order_by("last_first")
...
class DepartmentProgramsPage(Page):
    ...
    def get_context(self, request):
        ...
        programs = Program.objects.filter(active=True, department=dept_abbrev).order_by(
            "degree_conferred", "name"
        )
        context["programs"] = programs
        context.update(get_dept_sidebar_context(self))
        return context
...
class DepartmentStandardPage(Page):
    ...
    page_description = (
        "A standard content page within a department sub-site (shows dept sidebar)."
    )
